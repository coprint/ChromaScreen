...
class CounterInputFloat(Gtk.Box):
    ...
    def on_button_clicked(self, widget, value):
            ...
            current_value = float(self.label.get_label())
            ...
            if (self.multiplier  == 0.001):
                self.label.set_label('{:.3f}'.format(new_value))
            else:
                self.label.set_label('{:.2f}'.format(new_value))
    def updateValue(self, value):
        
        if (self.multiplier  == 0.001):
                self.label.set_label('{:.3f}'.format(value))
        else:
                self.label.set_label('{:.2f}'.format(value))
